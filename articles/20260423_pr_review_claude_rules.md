...
```mermaid
graph TD
    A[毎週月曜 朝9時] --> B[Claude Codeルーチン起動]
    B --> C[STEP1: 直近1週間のPR番号を収集]
    C --> D[STEP2: PRごとに並列サブエージェントでレビュー収集]
    D --> E[STEP3: 現在のルールファイルを読み込む]
    E --> F[STEP4: ギャップ分析]
    F -->|更新なし| G[タスク終了]
    F -->|更新あり| H[STEP5: ルール更新 & PR作成]
    H --> I[STEP6: 変更行にインラインコメントを追加]
```
...
:::message
ルーチンの実行回数にはプランごとの上限があります。週次実行であれば上限に引っかかることはありません。
:::
...
:::message
**人間によるレビューを挟む**
自動生成されたルールがすべて適切とは限りません。作成されたPRを人間がレビューしてからマージする運用を推奨します。PRベースでルール変更を管理することで、チームの合意を経てルールが確定します。
:::
